<div
 ...
 class="flex flex-col transition-all h-full overflow-hidden">
    ...
    <main
    class=" overflow-y-scroll overflow-hidden grow  h-full relative " style="contain:content">

        @if($conversation)
            @foreach ($conversation as $key=> $conversation)
                 {{-- chatlist  --}}
        <ul class="p-2 grid w-full spacey-y-2">
            <li id="conversation-{{$conversation->id}}" wire:key='{{$conversation->id}}'
            class="{{$conversation->id == $selectedConversation?->id ? 'bg-gray-100' : ''}} py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2">
                <a href="{{route('chat' , $conversation->id)}}" class="shrink-0">
                    <x-avatar src="https://avatar.iran.liara.run/public/{{$key }}" />
                </a>
                    ...
                    <a href="{{route('chat' , $conversation->id)}}" class="col-span-11 border-b pb-2 border-gray-200 relative overflow-hidden truncate leading-5 w-full flex-nowrap p-1">
                        ...
                        <div class="flex gap-x-2 items-center">
                            ...
                            @endif
                             ...
                             <span class="font-bold p-px px-2 text-xs shrink-0 rounded-full bg-blue-500 text-white">{{ $conversation->unreadMessagesCount() }}</span>
                             @endif
                        </div>
                    </a>
